...
app.controller('ProjectController', function ($log, $rootScope, $scope, $cookies, $modal, $location, $http, AlertService, AuthService, ProjectService) {
    	...
    	if($scope.form.$valid) {
        	if($scope.edit) {
        		...
        		ProjectService.update($scope.project).success(function(data, status, headers, config) {
        			...
        			var projects =[];
        			angular.copy($scope.parents, projects);
        		...
        		$log.debug('prepare to create project');
        		ProjectService.create($scope.project).success(function(data, status, headers, config) {
        			$log.debug('create project finished: ' + JSON.stringify(data));
        			$scope.createModal.destroy();
        			
    				var projects = [];
    				angular.copy($scope.projects, projects);
    				projects.unshift(data);
    				if (projects.length > $scope.pagingOptions.pageSize) {
    					projects.pop();
    				}
    				$scope.setPagingData(projects);
            		...
            		if(status === 409) {
	    				$scope.error = "Project already Exsits";
	    			}else {
	    				AlertService.error('Failed to create project, try again later.');
	    			}
            	});
