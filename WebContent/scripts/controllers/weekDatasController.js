...
app.controller('WeekDatasController', function ($log, $rootScope, $scope, $cookies, $location,$modal, AlertService, AuthService, ProjectService, WeekDataService) {
	$log.debug("WeekDatasController called.");
	...
	$scope.doGotoActionForEntity = function (entity) {
		...
		$scope.gridOptions = {
		    ...
		    $scope.getPagedDataAsync = function(condition, currentPage, pageSize) {
		        ...
		        // get data count
		    	WeekDataService.count(condition).success(function(data, status, headers, config) {
		    		$log.debug('total = ' + data.total);
		            $scope.totalServerItems = data.total;
		    	}).error(function(data, status, headers, config) {
		    		AlertService.error('Failed to load weekData number, try again later.');
		    	});
		    	
		    	// get data
		    	WeekDataService.list(condition).success(function(data, status, headers, config) {
		    		$log.debug('weekDatas = ' + JSON.stringify(data));
		    		$scope.setPagingData(data);
		    	}).error(function(data, status, headers, config) {
		    		AlertService.error('Failed to load weekDatas list, try again later.');
		    	});
		    };
