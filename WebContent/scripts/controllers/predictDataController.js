...
app.controller('PredictDataController', function ($log, $rootScope, $scope, $cookies, $location,$modal, AlertService, AuthService, ProjectService, PredictDataService) {
	$log.debug("PredictDataController called.");
	...
	$scope.doGotoActionForEntity = function (entity) {
		...
		$scope.gridOptions = {
		    ...
		    $scope.getPagedDataAsync = function(condition, currentPage, pageSize) {
		        ...
		        // get data count
		    	PredictDataService.count(condition).success(function(data, status, headers, config) {
		    	...
		    	// get data
		    	PredictDataService.list(condition).success(function(data, status, headers, config) {
